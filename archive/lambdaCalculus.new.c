...
LO_Substitute ( ListObject *lambdaParameters, ListObject *funcCallValues )
{
    while ( lambdaParameters && funcCallValues )
    {
        if ( lambdaParameters->LAttribute & ( LIST | LIST_NODE ) )
        {
            ...
            lambdaParameters = _LO_First ( lambdaParameters ) ; 
        }
        else if ( funcCallValues->LAttribute & ( LIST | LIST_NODE ) )
        {
            if ( lambdaParameters->LAttribute & ( LIST | LIST_NODE ) ) lambdaParameters = _LO_First ( lambdaParameters ) ; 
...
#if 0
ListObject *
_LO_Read_DoWord ( Word * word, int64 qidFlag, int64 scwi )
{
    ...
    {
        SetState ( _LC_, ( LC_READ ), false ) ; // let the value be pushed in this case because we need to pop it below
        Word_Eval ( word ) ;
        token1 = ( byte * ) DataStack_Pop ( ) ;
        SetState ( _LC_, ( LC_READ ), true ) ;
...
ListObject *
_LO_Read_DoSplice ( ListObject *l0, ListObject *lnew )
{
    ListObject *lreturn = 0 ;
    ...
    return lreturn ; //break ;
}

ListObject *
_LO_Read_DoToken ( byte * token, int64 qidFlag, int64 scwi )
{
    Context *cntx = _Context_ ;
    Lexer *lexer = cntx->Lexer0 ;
    ListObject *l0 ;
    ...
    if ( word )
    {
        l0 = _LO_Read_DoWord ( word, qidFlag, scwi ) ;
    }
    else
    {
...
ListObject *
_LO_Read_DoListObject ( ListObject * l0, ListObject * lnew, int64 qidFlag, int64 scwi )
{
    ListObject *lreturn ;
    l0->W_SC_Index = scwi ;
    ...
    {
        lreturn = _LO_Read_DoSplice ( l0, lnew ) ; //break ;
        //else lreturn = lnew ;
    }
    else if ( lnew )
    {
...
next:
        ...
        if ( token )
        {
            ...
            else
            {
                ...
                word = _LO_FindWord ( 0, token, 0 ) ;
                if ( qidFlag ) SetState ( cntx->Finder0, QID, false ) ;
                if ( word ) //&& ( ! GetState ( _LC_, LC_READ_LAMBDA_LOCALS ) ) )
                {
                    word->W_RL_Index = lexer->TokenStart_ReadLineIndex ;
                    DEBUG_SETUP ( word ) ;
                    if ( qidFlag ) SetState ( word, QID, true ) ;
...
_LO_CompileOrInterpret ( ListObject *lfunction, ListObject *ldata )
{
    ...
    else
    {
        ...
        {
            if ( GetState ( _LC_, LC_INTERP_DONE ) )
                return ;
            _LO_CompileOrInterpret_One ( ldata ) ; // research : how does CAttribute get set to T_NIL?
        }
        if ( lfword && ( ! ( lfword->CAttribute2 & LISP_CFRTIL ) ) )
            _LO_CompileOrInterpret_One ( lfword ) ; // ( ! ( lfword->CAttribute & LISP_CFRTIL ) ) : don't do it twice (see above)
    }
    _LC_->LastInterpretedWord = lfword ;
}
...
_LO_New_RawStringOrLiteral ( Lexer *lexer, byte *token, int64 qidFlag )
{
    if ( GetState ( lexer, KNOWN_OBJECT ) )
    {
        ...
        if ( ( ! qidFlag ) && ( lexer->TokenType & ( T_RAW_STRING ) ) )
        {
            ...
            _Namespace_DoAddWord ( _LC_->LispTemporariesNamespace, word, 0 ) ; // nb. here not in _DObject_New :: only for ( ! qidFlag ) && ( lexer->TokenType & T_RAW_STRING )
            word->Lo_Value = ( int64 ) word->Lo_Name ;
        }
        word->Lo_CfrTilWord = word ;
        if ( qidFlag )
            word->CAttribute &= ~ T_LISP_SYMBOL ;
        return word ;
    }
    else
    {
...
_LO_New ( uint64 ltype, uint64 ctype, uint64 ctype2, byte *value, Word *word, uint64 allocType, int64 rl_Index, int64 scwi )
{
    ...
    else if ( ltype & LIST_NODE )
        l0->S_SymbolList = ( dllist * ) value ;
    if ( word )
    {
...
{
    if ( _LC_ )
        _LC_->ItemQuoteState |= QUOTED ;
    else
        Lexer_CheckMacroRepl ( _Context_->Lexer0 ) ;
}
...
{
    if ( _LC_ )
        _LC_->ItemQuoteState |= QUASIQUOTED ;
    else
        Lexer_CheckMacroRepl ( _Context_->Lexer0 ) ;
}
...
LO_BeginBlock ( )
{
    if ( ! Compiler_BlockLevel ( _Compiler_ ) )
        _LC_->SavedCodeSpace = _Q_CodeByteArray ;
    CfrTil_BeginBlock ( ) ;
}
...
LO_EndBlock ( )
{
    ...
    if ( _LC_ && _LC_->SavedCodeSpace )
    {
        ...
        if ( ! Compiler_BlockLevel ( compiler ) )
            Set_CompilerSpace ( _LC_->SavedCodeSpace ) ;
...
_LO_Apply_Arg ( ListObject **pl1, int64 i )
{
    ...
    else if ( ( l1->CAttribute & NON_MORPHISM_TYPE ) ) // and literals, etc.
    {
        ...
        if ( ( word->Name[0] == '\"' ) || ( ! _Lexer_IsTokenForwardDotted ( cntx->Lexer0, l1->W_RL_Index + Strlen ( word->Name ) - 1 ) ) ) // ( word->Name[0] == '\"' ) : sometimes strings have ".[]" chars within but are still just strings
        {
            ...
            else if ( baseObject && baseObject->StackPushRegisterCode )
                SetHere ( baseObject->StackPushRegisterCode, 1 ) ;
    ...
    else if ( ( l1->Name[0] == '[' ) )
    {
        ...
        if ( arrayBaseObject )
        {
            ...
            if ( ( ! arrayBaseObject->ArrayDimensions ) )
                CfrTil_Exception ( ARRAY_DIMENSION_ERROR, 0, QUIT ) ;
            if ( interp->CurrentObjectNamespace )
                objSize = interp->CurrentObjectNamespace->Size ; //_CfrTil_VariableValueGet ( _Context_->Interpreter0->CurrentClassField, ( byte* ) "size" ) ;
            if ( ! objSize )
            {
            ...
            do
            {
                ...
                if ( Do_NextArrayWordToken ( word, token, arrayBaseObject, objSize, svcm, &variableFlag ) )
                    break ;
            }
            while ( l1 = LO_Next ( l1 ) ) ;
            *pl1 = l1 ;
            ...
            if ( CompileMode )
            {
                ...
                if ( Is_DebugModeOn )
                    Word_PrintOffset ( word, increment, baseObject->AccumulatedOffset ) ;
                if ( baseObject->StackPushRegisterCode )
                    SetHere ( baseObject->StackPushRegisterCode, 1 ) ;
                Compile_Move_Reg_To_Reg ( RegOrder ( i ++ ), ACC ) ;
                _Debugger_->PreHere = baseObject->Coding ;
...
_LO_Apply_ArgList ( ListObject *l0, Word *word )
{
    ...
    if ( l0 )
    {
        ...
        if ( word->CAttribute & ( DLSYM_WORD | C_PREFIX ) )
            Set_CompileMode ( true ) ;
        _Debugger_->PreHere = Here ;
        for ( i = 0, l1 = _LO_First ( l0 ) ; l1 ; l1 = LO_Next ( l1 ) )
            i = _LO_Apply_Arg ( &l1, i ) ;
        Set_CompileMode ( true ) ;
        _Debugger_->PreHere = Here ;
...
LC_CompileRun_C_ArgList ( Word *word ) // C protocol - x64 : left to right arguments put into registers
{
    ...
    {
        if ( ( ! token ) || strcmp ( "(", ( char * ) token ) )
            Error ( "", "Syntax error : C RTL Args : no '('", ABORT ) ; // should be '('
    }
    lc->LispParenLevel = 1 ;
    if ( word->CAttribute & ( C_PREFIX | C_PREFIX_RTL_ARGS ) )
    {
...
_LO_PrintOneToString ( ListObject *l0, byte *buffer, int64 in_a_LambdaFlag, int64 printValueFlag )
{
    ...
    if ( l0 )
    {
        ...
        else if ( l0->LAttribute & T_LISP_SYMBOL )
        {
            ...
            else if ( printValueFlag ) //&& GetState ( _LC_, ( PRINT_VALUE ) ) )
            {
                if ( *l0->Lo_PtrToValue != ( uint64 ) nil )
                {
                    ...
                    {
                        if ( _Q_->Verbosity > 2 )
                            snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s = 0x%016lx", l0->Lo_CfrTilWord->Lo_Name, ( int64 ) l0->Lo_CfrTilWord ) ;
                        else
                            snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s", l0->Lo_Name ) ;
                    }
                ...
                {
                    if ( _Q_->Verbosity > 2 )
                        snprintf ( ( char * ) buffer, BUFFER_SIZE, " nil: %s", l0->Lo_Name ) ;
                    else
                        snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s", l0->Lo_Name ) ;
                }
            }
        ...
        {
            if ( l0->State & UNQUOTED )
                snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s", ( char * ) l0->Lo_String ) ;
            else
                snprintf ( ( char * ) buffer, BUFFER_SIZE, " \"%s\"", ( char * ) l0->Lo_String ) ;
        }
        ...
        {
            if ( _Context_->System0->NumberBase == 16 )
                snprintf ( ( char * ) buffer, BUFFER_SIZE, " 0x%016lx", ( uint64 ) l0->Lo_UInteger ) ;
            else
            {
        ...
        else if ( l0->LAttribute & LITERAL )
        {
            ...
            {
                format = ( ( ( int64 ) l0->Lo_Integer ) < 0 ) ? ( byte * ) " 0x%016lx" : ( byte * ) " %ld" ;
                if ( ( l0->Lo_Integer < 0 ) || ( _Context_->System0->NumberBase == 16 ) )
                    snprintf ( ( char * ) buffer, BUFFER_SIZE, " 0x%016lx", ( uint64 ) l0->Lo_UInteger ) ;
                else
                    snprintf ( ( char * ) buffer, BUFFER_SIZE, ( char * ) format, l0->Lo_Integer ) ;
            }
        ...
        {
            if ( l0->Lo_CfrTilWord && l0->Lo_CfrTilWord->Lo_Name )
                snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s", l0->Lo_CfrTilWord->Lo_Name ) ;
            else if ( l0->Name )
                snprintf ( ( char * ) buffer, BUFFER_SIZE, " %s", l0->Name ) ;
        }
    }
done:
    if ( _LC_ )
...
LO_Car ( ListObject *l0 )
{
    ...
    if ( lfirst->LAttribute & ( LIST_NODE | LIST ) )
        return LO_CopyOne ( _LO_First ( lfirst ) ) ; //( ListObject * ) lfirst ;
    else
        return LO_CopyOne ( lfirst ) ;
}
...
LO_Cdr ( ListObject *l0 )
{
    ListObject *lfirst = _LO_Next ( l0 ) ;
    if ( lfirst->LAttribute & ( LIST_NODE | LIST ) )
        return _LO_Next ( _LO_First ( lfirst ) ) ; //( ListObject * ) lfirst ;
...
_LO_CfrTil ( ListObject *lfirst )
{
    ...
    for ( ldata = _LO_Next ( lfirst ) ; ldata ; ldata = _LO_Next ( ldata ) )
    {
        ...
        else if ( String_Equal ( ldata->Name, ( byte * ) ";" ) && ( ! GetState ( cntx, C_SYNTAX ) ) )
        {
            ...
            Interpreter_InterpretAToken ( cntx->Interpreter0, ldata->Name, ldata->W_RL_Index ) ;
    }
...
_LO_First ( ListObject *l0 )
{
    ...
    {
        if ( l0->LAttribute & ( LIST | LIST_NODE ) )
            return ( ListObject * ) dllist_First ( ( dllist * ) ( dllist * ) l0->Lo_List ) ;
        else
            return l0 ;
    }
    return 0 ;
...
_LO_Last ( ListObject *l0 )
{
    ...
    {
        if ( l0->LAttribute & ( LIST | LIST_NODE ) )
            return ( ListObject * ) dllist_Last ( ( dllist * ) l0->Lo_List ) ;
        else
            return l0 ;
    }
    return 0 ;
...
_LO_CopyOne ( ListObject *l0, uint64 allocType )
{
    ...
    if ( l0 )
    {
        if ( l0->LAttribute & ( LIST | LIST_NODE ) )
        {
            ...
            if ( l0->LAttribute & LIST_NODE )
                l1 = _DataObject_New ( T_LC_NEW, 0, 0, LIST_NODE, 0, LIST_NODE, 0, ( int64 ) l1, 0, - 1, - 1 ) ;
        }
        else
            l1 = _LO_AllocCopyOne ( l0, allocType ) ;
    }
    return l1 ;
}
...
_LO_Copy ( ListObject *l0, uint64 allocType )
{
    ...
    {
        if ( l0->LAttribute & ( LIST | LIST_NODE ) )
            lnew = _LO_ListNode_Copy ( l0, allocType ) ;
        for ( l0 = _LO_First ( l0 ) ; l0 ; l0 = _LO_Next ( l0 ) )
        {
...
_LO_ReadEvalPrint_ListObject ( int64 parenLevel, int64 continueFlag )
{
    ...
    if ( ! parenLevel )
        ...
        CfrTil_InitSourceCode_WithCurrentInputChar ( _CfrTil_ ) ;
...
{
    if ( lc && ( lc->SaveStackPointer ) )
        _Dsp_ = lc->SaveStackPointer ;
}
...
_LC_Init ( LambdaCalculus *lc, int64 newFlag )
{
    ...
    lc->QuoteState = 0 ;
    lc->LispParenLevel = 0 ;
    if ( newFlag || ( ! lc->QuoteStateStack ) )
        lc->QuoteStateStack = Stack_New ( 64, LISP_TEMP ) ;
    else
        _Stack_Init ( lc->QuoteStateStack, 64 ) ;
    lc->State = 0 ;
    DebugShow_On ;
...
LC_Clear ( int64 deleteFlag )
{
    ...
    if ( lc )
    {
        ...
        if ( deleteFlag )
            _LC_ = 0 ; // this maybe shouldn't be used as a flag
    }
}
