...
enum
{ Undefined, Data, Long, Double, String, Symbol, Pair, _Array, Array,
    Expr,
    Form, Fixed, Subr, Variable, Env, Context
};
...
getType ( oop obj )
{
    return obj ? ( ( ( long ) obj & 1 ) ? Long : ptr2hdr ( obj )->type ) :
	Undefined;
}
#else
static inline int
...
static inline oop
_checkType ( oop obj, int type, char *name, char *file, int line )
{
...
static oop s_define = nil,
    ...
    s_dot = nil,
    s_bracket = nil,
    s_brace = nil;		// , s_in= nil;
static oop f_lambda = nil,
    f_let = nil,
    f_quote = nil,
    f_set = nil,
    f_define;
...
static oop
_newString ( size_t len )
{
    wchar_t *gstr =
	( wchar_t * ) _newBits ( -1, sizeof ( wchar_t ) * ( len + 1 ) );

    GC_PROTECT ( gstr );	/* + 1 to ensure null terminator */
...
static oop
newStringN ( wchar_t * cstr, size_t len )
{
...
static oop
newPairFrom ( oop head, oop tail, oop source )
{
    oop obj = newOops ( Pair );
...
{
    int cap = size ? size : 1;
    oop elts = _newOops ( _Array, sizeof ( oop ) * cap );

    GC_PROTECT ( elts );
    oop obj = newOops ( Array );
...
static oop
arrayAtPut ( oop array, int index, oop val )
{
    if ( is ( Array, array ) )
    {
	int size = arrayLength ( array );
...
static oop
arrayAppend ( oop array, oop val )
{
    return arrayAtPut ( array, arrayLength ( array ), val );
}

static oop
arrayInsert ( oop obj, size_t index, oop value )
{
...
static oop
oopAtPut ( oop obj, int index, oop value )
{
    if ( obj && !isLong ( obj ) && !GC_atomic ( obj ) )
    {
	int size = GC_size ( obj ) / sizeof ( oop );
...
newExpr ( oop defn, oop ctx )
{
    ...
    set ( obj, Expr, profile, newLong ( 0 ) );
    GC_UNPROTECT ( obj );
    return obj;
}

static oop
newForm ( oop fn, oop sym )
{
...
static oop
newSubr ( imp_t imp, wchar_t * name )
{
...
static oop
newVariable ( oop name, oop value, oop env, int index )
{
    oop obj = newOops ( Variable );
    ...
    set ( obj, Variable, type, 0 );
    GC_UNPROTECT ( obj );
    return obj;
}

static oop
newEnv ( oop parent, int level, int offset )
{
    oop obj = newOops ( Env );
    ...
    set ( obj, Env, level,
	  newLong ( ( nil ==
		      parent ) ? 0 : getLong ( get ( parent, Env,
						     level ) ) + level ) );
    set ( obj, Env, offset, newLong ( offset ) );
    set ( obj, Env, bindings, newArray ( 0 ) );
    GC_UNPROTECT ( obj );
    return obj;
}

static oop
newBaseContext ( oop home, oop caller, oop env )
{
    oop obj = newOops ( Context );

    GC_PROTECT ( obj );
    set ( obj, Context, home, home );
    set ( obj, Context, env, env );
    set ( obj, Context, bindings, newArray ( 0 ) );
    GC_UNPROTECT ( obj );
    return obj;
}

static oop
newContext ( oop home, oop caller, oop env )
{
    oop obj = nil;
...
#if 0
    if ( ( nil != caller )
	 && ( nil != ( obj = get ( caller, Context, callee ) ) ) )
    {
	set ( obj, Context, home, home );
	set ( obj, Context, env, env );
	return obj;
...
static oop
findLocalVariable ( oop env, oop name )
{
...
static oop
lookup ( oop env, oop name )
{
    oop var = findVariable ( env, name );

    if ( nil == var )
	fatal ( "undefined variable: %ls", get ( name, Symbol, bits ) );
    return get ( var, Variable, value );
}

static oop
define ( oop env, oop name, oop value )
{
    oop bindings = get ( env, Env, bindings );
    ...
    oop var = newVariable ( name, value, env, off );

    GC_PROTECT ( var );
    arrayAppend ( bindings, var );
    GC_UNPROTECT ( var );
...
isPrint ( int c )
{
    return ( 0 <= c && c <= 127 && ( CHAR_PRINT & chartab[c] ) )
	|| ( c >= 128 );
}
...
isAlpha ( int c )
{
    return ( 0 <= c && c <= 127 && ( CHAR_ALPHA & chartab[c] ) )
	|| ( c >= 128 );
}
...
isDigit10 ( int c )
{
    return ( 0 <= c && c <= 127 && ( CHAR_DIGIT10 & chartab[c] ) );
}

static int
isDigit16 ( int c )
{
    return ( 0 <= c && c <= 127 && ( CHAR_DIGIT16 & chartab[c] ) );
}

static int
isLetter ( int c )
{
    return ( 0 <= c && c <= 127 && ( CHAR_LETTER & chartab[c] ) )
	|| ( c >= 128 );
}
...
readList ( FILE * fp, int delim )
{
	...
	if ( obj == s_dot )
	{
	    obj = read ( fp );
	    if ( obj == DONE )
		fatal ( "missing item after ." );
	    tail = set ( tail, Pair, tail, obj );
	    obj = read ( fp );
	    if ( obj != DONE )
		fatal ( "extra item after ." );
	    goto eof;
	}
...
readChar ( wint_t c, FILE * fp )
{
	...
	switch ( c )
	{
	    ...
	    case 'u':
		{
		    ...
		    return ( digitValue ( a ) << 12 ) +
			( digitValue ( b ) << 8 ) +
			( digitValue ( c ) << 4 ) + digitValue ( d );
		}
	    case 'x':
		{
		    int x = 0;
...
read ( FILE * fp )
{
	...
	switch ( c )
	{
	    ...
	    case '`':
		{
			...
			obj = newPairFrom ( obj, nil, currentSource );
			obj =
			    newPairFrom ( s_quasiquote, obj, currentSource );
			GC_UNPROTECT ( obj );
		    }
		    return obj;
		}
  ...
  doDigits:	{
			...
			ungetwc ( c, fp );
			oop obj =
			    newDouble ( wcstod ( buffer_contents ( &buf ),
						 0 ) );
	    ...
	    case '[':
		{
		    ...
		    GC_PROTECT ( obj );
		    obj = newPairFrom ( s_bracket, obj, obj );
		    GC_UNPROTECT ( obj );
		    return obj;
		}
	    case ']':
	    ...
	    case '{':
		{
		    ...
		    GC_PROTECT ( obj );
		    obj = newPairFrom ( s_brace, obj, obj );
		    GC_UNPROTECT ( obj );
		    return obj;
		}
	    case '}':
	    ...
	    default:
		{
		    ...
		    fatal ( isPrint ( c ) ?
			    "illegal character: 0x%02x '%c'" :
			    "illegal character: 0x%02x", c, c );
		}
	}
    }
}
...
static void
doprint ( FILE * stream, oop obj, int storing )
{
    if ( !obj )
    {
	fprintf ( stream, "()" );
	...
	case Data:
		...
		// for (i= 0; i < j; ++i) fprintf(stream, " %02x",
		// ((unsigned
		// char *)obj)[i]);
		fprintf ( stream, ">" );
		break;
	...
	case Long:
	    fprintf ( stream, "%ld", getLong ( obj ) );
	    break;
	case Double:
	    fprintf ( stream, "%lf", getDouble ( obj ) );
	    break;
	case String:
	    {
		if ( !storing )
	...
	case Pair:
		...
		if ( nil != get ( obj, Pair, source ) )
		{
		    ...
		    fprintf ( stream, "<%ls:%ld>",
			      get ( path, String, bits ), getLong ( line ) );
		}
#endif
		fprintf ( stream, "(" );
		for ( ;; )
	...
	case Variable:
		...
		if ( nil != env )
		    fprintf ( stream, ";%ld+%ld",
			      getLong ( get ( env, Env, level ) ),
			      getLong ( get ( obj, Variable, index ) ) );
#endif
		break;
	    }
	case Env:
		...
		fprintf ( stream, "Env%s%s<%ld+%ld:",
			  ( ( nil == get ( obj, Env, parent ) ) ? "*" : "" ),
			  ( ( nil == get ( obj, Env, stable ) ) ? "=" : "" ),
			  is ( Long, level ) ? getLong ( level ) : -1,
			  is ( Long, offset ) ? getLong ( offset ) : -1 );
#if 0
	...
	default:
	    {
		oop name = lookup ( get ( globals, Variable, value ),
				    intern ( L"%type-names" ) );
		...
		if ( is ( Array, name ) )
		{
		    ...
		    if ( is ( Symbol, name ) )
		    {
			fprintf ( stream, "[34m%ls[m",
				  get ( name, Symbol, bits ) );
			break;
		    }
		}
...
print ( oop obj )
{
    doprint ( stdout, obj, 0 );
    fflush ( stdout );
}

static void
fdump ( FILE * stream, oop obj )
{
    doprint ( stream, obj, 1 );
    fflush ( stream );
}
...
static oop
concat ( oop head, oop tail )
{
    if ( !is ( Pair, head ) )
	return tail;
    tail = concat ( getTail ( head ), tail );
    GC_PROTECT ( tail );
    head = newPairFrom ( getHead ( head ), tail, head );
    GC_UNPROTECT ( tail );
    return head;
}

static void
setSource ( oop obj, oop src )
{
    if ( !is ( Pair, obj ) )
	return;
    if ( nil == get ( obj, Pair, source ) )
	set ( obj, Pair, source, src );
...
expand ( oop expr, oop env )
{
    ...
    if ( is ( Pair, expr ) )
    {
	oop head = expand ( getHead ( expr ), env );
	...
	if ( is ( Symbol, head ) )
	{
	    ...
	    {
		oop args = newPairFrom ( env, getTail ( expr ), expr );

		GC_PROTECT ( args );
		head = apply ( get ( val, Form, function ), args, nil );
		GC_UNPROTECT ( args );
											 ...
											 */  )
	{
	    ...
	    buffer_appendAll ( &buf,
			       get ( getHead ( getHead ( tail ) ), Symbol,
				     bits ) );
    ...
    {
	oop fn = arrayAt ( get ( expanders, Variable, value ),
			   getType ( expr ) );
...
exlist ( oop list, oop env )
{
    ...
    GC_PROTECT ( tail );
    head = newPairFrom ( head, tail, list );
    GC_UNPROTECT ( tail );
    GC_UNPROTECT ( head );
    return head;
}
...
static void
define_bindings ( oop bindings, oop innerEnv )
{
    GC_PROTECT ( bindings );
    while ( is ( Pair, bindings ) )
    {
	oop var = getHead ( bindings );
...
static oop
encode_bindings ( oop expr, oop bindings, oop outerEnv, oop innerEnv )
{
    if ( is ( Pair, bindings ) )
    {
	GC_PROTECT ( bindings );
	...
	GC_UNPROTECT ( val );
	GC_UNPROTECT ( var );
	oop rest = encode_bindings ( expr, getTail ( bindings ), outerEnv,
				     innerEnv );
...
static oop
encode_let ( oop expr, oop tail, oop env )
{
    oop args = car ( tail );
    ...
    set ( env, Env, offset,
	  newLong ( getLong ( get ( env2, Env, offset ) ) ) );
...
encode ( oop expr, oop env )
{
    ...
    if ( is ( Pair, expr ) )
    {
	oop head = encode ( getHead ( expr ), env );
	...
	{			// (lambda ENV params . body)
	    ...
	    tail = newPairFrom ( env, tail, expr );
	    GC_UNPROTECT ( env );
	}
	else if ( f_define == head )
	{
	    oop var = define ( get ( globals, Variable, value ), car ( tail ),
			       nil );
	...
	else if ( f_set == head )
	{
	    ...
	    if ( nil == var )
		fatal ( "set: undefined variable: %ls",
			get ( car ( tail ), Symbol, bits ) );
	    tail = enlist ( cdr ( tail ), env );
	    tail = newPairFrom ( var, tail, expr );
	}
	...
	if ( nil == val )
	    fatal ( "undefined variable: %ls", get ( expr, Symbol, bits ) );
	expr = val;
	if ( isGlobal ( expr ) )
	{
    ...
    {
	oop fn = arrayAt ( get ( encoders, Variable, value ),
			   getType ( expr ) );

	if ( nil != fn )
	{
	    oop args = newPair ( env, nil );

	    GC_PROTECT ( args );
	    args = newPair ( expr, args );
	    expr = apply ( fn, args, nil );
	    GC_UNPROTECT ( args );
	}
    }
    if ( opt_v > 1 )
...
static oop
enlist ( oop list, oop env )
{
    if ( !is ( Pair, list ) )
	return encode ( list, env );
...
static int
fprintSource ( FILE * stream, oop exp )
{
    if ( is ( Pair, exp ) )
    {
	...
	if ( nil != src )
	{
	    ...
	    {
		return fprintf ( stream, "%ls:%ld",
				 get ( path, String, bits ),
				 getLong ( line ) );
	    }
	}
    }
...
eval ( oop obj, oop ctx )
{
	...
	case Pair:
		...
		oop head = eval ( getHead ( obj ), ctx );

		GC_PROTECT ( head );
		if ( is ( Fixed, head ) )
		    head =
			apply ( get ( head, Fixed, function ),
				getTail ( obj ), ctx );
		else
		{
		    oop args = evlist ( getTail ( obj ), ctx );

		    GC_PROTECT ( args );
		    if ( opt_g )
			arrayAtPut ( traceStack, traceDepth++,
				     newPair ( head, args ) );
		    head = apply ( head, args, ctx );
		    GC_UNPROTECT ( args );
		    if ( opt_g )
			--traceDepth;
		}
	...
	case Variable:
	    {
		if ( isGlobal ( obj ) )
		    return get ( obj, Variable, value );
		int delta =
		    getLong ( get ( get ( ctx, Context, env ), Env, level ) )
		    -
		    getLong ( get
			      ( get ( obj, Variable, env ), Env, level ) );
		oop cx = ctx;

		while ( delta-- )
		    cx = get ( cx, Context, home );
		return arrayAt ( get ( cx, Context, bindings ),
				 getLong ( get ( obj, Variable, index ) ) );
	    }
	default:
	    {
		if ( opt_g )
		    arrayAtPut ( traceStack, traceDepth++, obj );
		oop ev = arrayAt ( get ( evaluators, Variable, value ),
				   getType ( obj ) );
...
evlist ( oop obj, oop ctx )
{
    ...
    GC_PROTECT ( head );
    oop tail = evlist ( getTail ( obj ), ctx );

    GC_PROTECT ( tail );
    // head= newPairFrom(head, tail, obj); GC_UNPROTECT(tail);
    // GC_UNPROTECT(head);
...
static oop
apply ( oop fun, oop arguments, oop ctx )
{
    if ( opt_v > 2 )
    {
	printf ( "APPLY " );
	...
	case Expr:
		...
		ctx = newContext ( get ( fun, Expr, ctx ), ctx, env );
		GC_PROTECT ( ctx );
		...
		while ( is ( Pair, formals ) )
		{
		    ...
		    arrayAtPut ( locals,
				 getLong ( get
					   ( getHead ( formals ), Variable,
					     index ) ), getHead ( args ) );
		    formals = getTail ( formals );	// xxx formals should be
		    // in env with fixed
		    // argument arity in defn
		    args = getTail ( args );	// xxx args should be set
		    // up in
		    // the callee context
		}
		...
		{
		    arrayAtPut ( locals,
				 getLong ( get ( formals, Variable, index ) ),
				 args );
		    args = nil;
		}
		if ( nil != args )
		...
		{
		    if ( opt_g )
			arrayAtPut ( traceStack, traceDepth - 1,
				     getHead ( body ) );
		...
		GC_UNPROTECT ( ctx );
		GC_UNPROTECT ( defn );
		if ( nil != get ( env, Env, stable ) )
		    set ( ctx, Context, callee, nil );
		return ans;
	    }
	case Fixed:
	    {
	...
	default:
	    {
		oop args = arguments;
		oop ap = arrayAt ( get ( applicators, Variable, value ),
				   getType ( fun ) );
...
static void
arity1 ( oop args, char *name )
{
    if ( !is ( Pair, args ) || is ( Pair, getTail ( args ) ) )
	arity ( args, name );
}

static void
arity2 ( oop args, char *name )
{
    if ( !is ( Pair, args ) || !is ( Pair, getTail ( args ) )
	 || is ( Pair, getTail ( getTail ( args ) ) ) )
	arity ( args, name );
}

static void
arity3 ( oop args, char *name )
{
    if ( !is ( Pair, args ) || !is ( Pair, getTail ( args ) )
	 || !is ( Pair, getTail ( getTail ( args ) ) )
	 || is ( Pair, getTail ( getTail ( getTail ( args ) ) ) ) )
	arity ( args, name );
}
...
subr ( set )
{
    ...
    int delta =
	getLong ( get ( get ( ctx, Context, env ), Env, level ) ) -
	getLong ( get ( get ( var, Variable, env ), Env, level ) );
    oop cx = ctx;

    while ( delta-- )
	cx = get ( cx, Context, home );
    return arrayAtPut ( get ( cx, Context, bindings ),
			getLong ( get ( var, Variable, index ) ), val );
}
...
subr ( let )
{
	...
	if ( is ( Pair, binding ) )
	{
	    ...
	    arrayAtPut ( locals, getLong ( get ( var, Variable, index ) ),
			 tmp );
	}
	bindings = getTail ( bindings );
    }
...
subr ( define )
{
    ...
    if ( is ( Form, expr ) )
	expr = get ( value, Form, function );
    if ( is ( Expr, expr ) && ( nil == get ( expr, Expr, name ) ) )
	set ( expr, Expr, name, get ( var, Variable, name ) );
    return value;
}
...
subr ( definedP )
{
    ...
    if ( nil == theenv )
	theenv = get ( globals, Variable, value );
    ...
    static subr ( sub )
{
    ...
    {
	if ( isLong ( lhs ) )
	    return newLong ( -getLong ( lhs ) );
	if ( isDouble ( lhs ) )
	    return newDouble ( -getDouble ( lhs ) );
	...
	if ( isDouble ( rhs ) )
	    return newDouble ( ( double ) getLong ( lhs ) -
			       getDouble ( rhs ) );
    }
    if ( isDouble ( lhs ) )
    {
	if ( isDouble ( rhs ) )
	    return newDouble ( getDouble ( lhs ) - getDouble ( rhs ) );
	if ( isLong ( rhs ) )
	    return newDouble ( getDouble ( lhs ) -
			       ( double ) getLong ( rhs ) );
...
subr ( mod )
{
	...
	if ( isDouble ( rhs ) )
	    return
		newDouble ( fmod ( getDouble ( lhs ), getDouble ( rhs ) ) );
	if ( isLong ( rhs ) )
	    return
		newDouble ( fmod
			    ( getDouble ( lhs ),
    ...
    static int equal ( oop lhs, oop rhs )
{
	...
	case Long:
	    ...
	    {
		case Long:
		    ans = ( getLong ( lhs ) == getLong ( rhs ) );
		    break;
		case Double:
		    ans = ( ( double ) getLong ( lhs ) == getDouble ( rhs ) );
		    break;
	    }
	    break;
	case Double:
	    ...
	    {
		case Long:
		    ans = ( getDouble ( lhs ) == ( double ) getLong ( rhs ) );
		    break;
		case Double:
		    ans = ( getDouble ( lhs ) == getDouble ( rhs ) );
		    break;
	    }
	    break;
	case String:
	    ans = ( is ( String, rhs )
		    && !wcscmp ( get ( lhs, String, bits ),
				 get ( rhs, String, bits ) ) );
	    break;
	default:
	    ans = ( lhs == rhs );
...
subr ( open )
{
    ...
    if ( !is ( String, arg ) )
    {
	fprintf ( stderr, "open: non-string argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
    ...
    if ( is ( String, cadr ( args ) ) )
	mode = wcs2mbs ( get ( cadr ( args ), String, bits ) );
    if ( is ( Long, caddr ( args ) ) )
	wide = getLong ( caddr ( args ) );
...
subr ( close )
{
    ...
    if ( !isLong ( arg ) )
    {
	fprintf ( stderr, "close: non-integer argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
...
subr ( getb )
{
    ...
    if ( nil == arg )
	arg = get ( input, Variable, value );
    if ( !isLong ( arg ) )
    {
	fprintf ( stderr, "getb: non-integer argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
...
subr ( getc )
{
    ...
    if ( !isLong ( arg ) )
    {
	fprintf ( stderr, "getc: non-integer argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
    FILE *stream = ( FILE * ) getLong ( arg );
...
subr ( putb )
{
    ...
    if ( nil == arg )
	arg = get ( output, Variable, value );
    if ( !isLong ( chr ) )
    {
	fprintf ( stderr, "putb: non-integer character: " );
	fdumpln ( stderr, chr );
	fatal ( 0 );
    }
    if ( !isLong ( arg ) )
    {
	fprintf ( stderr, "putb: non-integer argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
    FILE *stream = ( FILE * ) getLong ( arg );
...
subr ( putc )
{
    ...
    if ( !isLong ( chr ) )
    {
	fprintf ( stderr, "putc: non-integer character: " );
	fdumpln ( stderr, chr );
	fatal ( 0 );
    }
    if ( !isLong ( arg ) )
    {
	fprintf ( stderr, "putc: non-integer argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
    FILE *stream = ( FILE * ) getLong ( arg );
...
subr ( read )
{
    ...
    if ( !is ( String, arg ) )
    {
	fprintf ( stderr, "read: non-String argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
    ...
    fwide ( stream, 1 );
    beginSource ( path );
    oop head = newPairFrom ( nil, nil, currentSource ),
	tail = head;
...
subr ( eval )
{
    ...
    if ( nil == e )
	e = newEnv ( get ( globals, Variable, value ), 1, 0 );
    GC_PROTECT ( e );
    x = expand ( x, e );
    x = encode ( x, e );
    oop c = newBaseContext ( nil, nil, e );
...
{
    if ( !is ( Pair, args ) )
	fatal ( "too few arguments in: apply" );
    oop f = car ( args );
    oop a = args;
...
subr ( format )
{
	...
	case Undefined:
	    break;
	case Long:
	    arg = ( void * ) getLong ( oarg );
	    break;
	    // case Double: arg= (void *)getDouble(oarg); break;
	case String:
	    arg = ( void * ) get ( oarg, String, bits );
	    break;
	case Symbol:
	    arg = ( void * ) get ( oarg, Symbol, bits );
	    break;
	default:
	    arg = ( void * ) oarg;
	    break;
    ...
    if ( !( p = malloc ( sizeof ( wchar_t ) * size ) ) )
	return nil;
    for ( ;; )
    {
...
subr ( cons )
{
    ...
    return newPair ( lhs, rhs );	// (is(Pair, rhs) ?
    // newPairFrom(lhs, rhs,
    // rhs) : newPair(lhs, rhs));
}
...
subr ( string_length )
{
    ...
    if ( !is ( String, arg ) )
    {
	fprintf ( stderr, "string-length: non-String argument: " );
	fdumpln ( stderr, arg );
	fatal ( 0 );
    }
...
subr ( string_at )
{
    ...
    if ( !is ( String, arr ) )
    {
	fprintf ( stderr, "string-at: non-String argument: " );
	fdumpln ( stderr, arr );
	fatal ( 0 );
    }
	...
	return newLong ( get ( arr, String, bits )[idx] );
    return nil;
}
...
subr ( set_string_at )
{
	...
	while ( len <= idx )
	    len *= 2;
	set ( arr, String, bits,
	      ( wchar_t * ) GC_realloc ( get ( arr, String, bits ),
...
static
subr ( string_copy )		// string from len
{
    oop str = car ( args );
...
static
subr ( string_compare )		// string substring offset=0
				// length=strlen(substring)
{
    ...
    if ( off < 0 || len < 0 )
	return newLong ( -1 );
    if ( off >= stringLength ( str ) )
	return newLong ( -1 );
    return
	newLong ( wcsncmp
		  ( get ( str, String, bits ) + off,
		    get ( arg, String, bits ), len ) );
}
...
subr ( symbol_compare )
{
    ...
    return
	newLong ( wcscmp
		  ( get ( str, Symbol, bits ), get ( arg, Symbol, bits ) ) );
}
...
static
subr ( array_compare )		// array subarray offset=0
				// length=arrlen(subarray)
{
    oop arr = car ( args );

    if ( !is ( Array, arr ) )
    {
	fprintf ( stderr, "array-compare: non-array argument: " );
	fdumpln ( stderr, arr );
	fatal ( 0 );
    }
    oop brr = cadr ( args );
	...
	if ( !isLong ( oof ) )
	{
	    fprintf ( stderr, "array-compare: non-integer offset: " );
	    fdumpln ( stderr, oof );
	    fatal ( 0 );
	}
	off = getLong ( oof );
	...
	if ( !isLong ( oln ) )
	{
	    fprintf ( stderr, "array-compare: non-integer length: " );
	    fdumpln ( stderr, oln );
	    fatal ( 0 );
	}
	len = getLong ( oln );
	if ( len < 0 || len > bln || off + len >= aln )
	    return newLong ( -1 );
    }
    else
    {
...
accessor ( byte, unsigned char ) accessor ( long, long )
#undef accessor
#if !defined(WIN32)
#include <sys/mman.h>
#endif
...
subr ( native_call )
{
    ...
    while ( is ( Pair, args ) && argc < 32 )
    {
        ...
        switch ( getType ( arg ) )
        {
            case Undefined:
                ...
                break ;
            case Long:
                argv.l[argc] = getLong ( arg ) ;
                break ;
            case Double:
                argc = ( argc + 1 ) & - 2 ;
                argv.l[argc ++] = ( ( long * ) arg )[0] ;
                argv.l[argc] = ( ( long * ) arg )[1] ;
                break ;
            case String:
                argv.l[argc] = ( long ) wcs2mbs ( get ( arg, String, bits ) ) ;
                break ;
            case Subr:
                argv.l[argc] = ( long ) get ( arg, Subr, imp ) ;
                break ;
            default:
                argv.l[argc] = ( long ) arg ;
                break ;
        }
        ++ argc ;
    }
    ...
    switch ( getType ( obj ) )
    {
        case Data:
            ...
            break ;
        case Long:
            addr = ( void * ) getLong ( obj ) ;
            break ;
        case Subr:
            addr = get ( obj, Subr, imp ) ;
            break ;
        default:
            fatal ( "call: cannot call object of type %i", getType ( obj ) ) ;
    }
    if ( size )
    {
#if !defined(WIN32)
        extern int getpagesize ( ) ;
        void *start = ( void * ) ( ( long ) addr & -( long ) getpagesize ( ) ) ; // round 
...
subr ( subr )
{
    ...
    {
        case String:
            name = get ( ptr, String, bits ) ;
            break ;
        case Symbol:
            name = get ( ptr, Symbol, bits ) ;
            break ;
        default:
            fatal ( "subr: argument must be string or symbol" ) ;
    }
    char *sym = wcs2mbs ( name ) ;
    void *addr = dlsym ( RTLD_DEFAULT, sym ) ;

    if ( ! addr )
        fatal ( "could not find symbol: %s", sym ) ;
...
subr ( times )
{
    ...
    oop real = newLong ( tv.tv_sec * 1000 + tv.tv_usec / 1000 ) ;

    GC_PROTECT ( real ) ;
    oop user =
        newLong ( ru.ru_utime.tv_sec * 1000 + ru.ru_utime.tv_usec / 1000 ) ;

    GC_PROTECT ( user ) ;
    oop syst =
        newLong ( ru.ru_stime.tv_sec * 1000 + ru.ru_stime.tv_usec / 1000 ) ;

    GC_PROTECT ( syst ) ;
    syst = newPair ( syst, nil ) ;
    user = newPair ( user, syst ) ;
    GC_UNPROTECT ( syst ) ;
    real = newPair ( real, user ) ;
    GC_UNPROTECT ( user ) ;
    GC_UNPROTECT ( real ) ;
    return real ;
}
...
static void
replFile ( FILE * stream, wchar_t * path )
{
    ...
    for ( ; ; )
    {
        ...
        obj = expand ( obj, env ) ;
        obj = encode ( obj, env ) ;
        oop ctx = newBaseContext ( nil, nil, env ) ;
        ...
        if ( opt_v )
        {
            ...
            GC_gcollect ( ) ;
            printf
                ( "%ld collections, %ld objects, %ld bytes, %4.1f%% fragmentation\n",
                ( long ) GC_collections, ( long ) GC_count_objects ( ),
                ( long ) GC_count_bytes ( ),
    ...
    if ( stats )
    {
        ...
        for ( obj = GC_first_object ( ) ; obj ; obj = GC_next_object ( obj ) )
        {
            ...
            if ( profile )
            {
                ...
                for ( jndex = nprofiles - 1 ; jndex > index ; -- jndex )
                    profiles[jndex] = profiles[jndex - 1] ;
                profiles[index] = ( struct profile ) {
                    profile, obj, source
                } ;
            }
        }
...
main ( int argc, char **argv )
{
    ...
    if ( ( fwide ( stdin, 1 ) <= 0 ) || ( fwide ( stdout, - 1 ) >= 0 )
        || ( fwide ( stderr, - 1 ) >= 0 ) )
    {
    ...
    {
        fprintf ( stderr,
            "Cannot set the locale.  Verify your LANG, LC_CTYPE, LC_ALL.\n" ) ;
        return 1 ;
    }
    ...
    expanders =
        define ( get ( globals, Variable, value ), intern ( L"*expanders*" ),
        nil ) ;
    encoders =
        define ( get ( globals, Variable, value ), intern ( L"*encoders*" ),
        nil ) ;
    evaluators =
        define ( get ( globals, Variable, value ), intern ( L"*evaluators*" ),
        nil ) ;
    applicators =
        define ( get ( globals, Variable, value ),
        intern ( L"*applicators*" ), nil ) ;

    traceStack = newArray ( 32 ) ;
    GC_add_root ( &traceStack ) ;

    backtrace =
        define ( get ( globals, Variable, value ), intern ( L"*backtrace*" ),
        nil ) ;
    input =
        define ( get ( globals, Variable, value ), intern ( L"*input*" ),
        nil ) ;
    output =
        define ( get ( globals, Variable, value ), intern ( L"*output*" ),
        nil ) ;

    currentPath = nil ;
    GC_add_root ( &currentPath ) ;
    currentLine = nil ;
    GC_add_root ( &currentLine ) ;
    currentSource = newPair ( nil, nil ) ;
    GC_add_root ( &currentSource ) ;
...
#undef _do
    ...
    arguments =
        define ( get ( globals, Variable, value ), intern ( L"*arguments*" ),
        tmp ) ;

    tmp = nil ;
    GC_UNPROTECT ( tmp ) ;

    f_set = lookup ( get ( globals, Variable, value ), s_set ) ;
    GC_add_root ( &f_set ) ;
    f_quote = lookup ( get ( globals, Variable, value ), s_quote ) ;
    GC_add_root ( &f_quote ) ;
    f_lambda = lookup ( get ( globals, Variable, value ), s_lambda ) ;
    GC_add_root ( &f_lambda ) ;
    f_let = lookup ( get ( globals, Variable, value ), s_let ) ;
    GC_add_root ( &f_let ) ;
    f_define = lookup ( get ( globals, Variable, value ), s_define ) ;
    GC_add_root ( &f_let ) ;
    ...
    if ( opt_v )
    {
        ...
        printf
            ( "%ld collections, %ld objects, %ld bytes, %4.1f%% fragmentation\n",
            ( long ) GC_collections, ( long ) GC_count_objects ( ),
            ( long ) GC_count_bytes ( ), GC_count_fragments ( ) * 100.0 ) ;
#endif
    }
