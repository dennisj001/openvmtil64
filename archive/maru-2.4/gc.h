...
GC_API void    *GC_stamp ( void *ptr, const char *file, long line,
			   const char *func );
...
GC_API void     GC_register_finaliser ( void *ptr,
...
GC_API inline void
GC_push_root ( struct GC_StackRoot *sr )
{
...
GC_API inline void
GC_pop_root ( struct GC_StackRoot *sr )
{
#if 0
  GC_stack_roots = sr->next;
#else				/* paranoid version for broken code warns
				 * of mismatched pops with a SEGV */
...
GC_API void
GC_push_root ( struct GC_StackRoot *sr, const char *name, const char *file,
	       int line )
{
  sr->next = GC_stack_roots;
  sr->name = name;
...
static int
GC_roots_include ( struct GC_StackRoot *roots, struct GC_StackRoot *root )
{
  while ( roots )
  {
    if ( roots == root )
...
GC_API void
GC_pop_root ( struct GC_StackRoot *sr, const char *name, const char *file,
	      int line )
{
  struct GC_StackRoot *nr = sr->next;
  ...
  if ( !sr->live )
  {
    fprintf ( stderr, "*** %s %d %s: STALE POP IN GC_pop_root\n", file,
	      line, name );
    goto die;
  }
  sr->live = 0;
  ...
  {
    fprintf ( stderr, "*** %s %d %s: CYCLE IN GC_pop_root\n", file, line,
	      name );
    goto die;
  }
  int             n = 0;
  ...
  while ( nr != gr )
  {
    if ( n++ > 10 )
    {
      fprintf ( stderr, "*** %s %d %s: LOOP IN GC_pop_root\n", file, line,
		name );
      goto die;
    }
    gr = gr->next;
...
die:
  ...
  while ( nr )
  {
    fprintf ( stderr, "* next root      = %p %s %ld %s\n", nr,
	      nr ? nr->file : 0, nr ? nr->line : 0, nr ? nr->name : 0 );
    nr = nr->next;
  }
  abort (  );
}
