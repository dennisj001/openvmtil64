...
#define LO_New( lType, object ) (ListObject *) _DataObject_New ( T_LC_NEW, 0, 0, 0, lType, 0, (int64) object, 0 )
        ...
        else if ( l0->LAttribute & ( LIST | LIST_NODE ) )
        {
            ...
            if ( lfirst )
            {
                if ( lfirst->LAttribute & ( T_LISP_SPECIAL | T_LISP_MACRO ) )
                {
                    ...
                    d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\n_LO_Eval : before LO_SpecialFunction : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), c_dd ( _LO_PRINT_TO_STRING ( locals ) ) ) ) ;
                    l0 = LO_SpecialFunction ( l0, locals ) ;
                    d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\n_LO_Eval : after LO_SpecialFunction : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), c_dd ( _LO_PRINT_TO_STRING ( locals ) ) ) ) ;
                    lc->LispParenLevel -- ;
                    goto done ;
                }
...
LO_SpecialFunction ( ListObject * l0, ListObject * locals )
{
    ...
    if ( lfirst = _LO_First ( l0 ) )
    {
        ...
        {
            d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\nLO_SpecialFunction : macro eval before : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), locals ? _LO_PRINT_TO_STRING ( locals ) : ( byte* ) "" ) ) ;
            macro = lfirst ;
            ...
            lfirst = _LO_First ( l0 ) ;
            macro = 0 ;
            d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\nLO_SpecialFunction : macro eval after : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), locals ? _LO_PRINT_TO_STRING ( locals ) : ( byte* ) "" ) ) ;
        }
        ...
        {
            d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\nLO_SpecialFunction : final eval before : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), locals ? _LO_PRINT_TO_STRING ( locals ) : ( byte* ) "nil" ) ) ;
            l0 = _LO_Eval ( l0, locals, 1 ) ;
            d0 ( if ( Is_DebugModeOn ) LO_Debug_ExtraShow ( 0, 0, 0, ( byte* ) "\nLO_SpecialFunction : final eval after : l0 = %s : locals = %s", c_dd ( _LO_PRINT_TO_STRING ( l0 ) ), locals ? _LO_PRINT_TO_STRING ( locals ) : ( byte* ) "nil" ) ) ;
        }
    }
    return l0 ;
}
...
_LO_Define ( byte * sname, ListObject * idNode, ListObject * locals )
{
    ...
    *word->Lo_PtrToValue = ( uint64 ) value ;
    ...
    l1 = _DataObject_New ( T_LC_NEW, word, 0, word->CAttribute, word->LAttribute, 0, ( int64 ) value, 0 ) ; // all words are symbols
...
_LO_MakeLambda ( ListObject * l0 )
{
    ...
    else word = _Word_New ( ( byte* ) "lambda", WORD_CREATE, 0, 1, 0, DICTIONARY ) ;
    args = l0 ;
    body0 = _LO_Next ( l0 ) ;
...
_LO_New_RawStringOrLiteral ( Lexer * lexer, byte * token, int64 qidFlag )
{
    if ( GetState ( lexer, KNOWN_OBJECT ) )
    {
        ...
        if ( ( ! qidFlag ) && ( lexer->TokenType & T_RAW_STRING ) )
        {
            ...
            *word->Lo_PtrToValue = ( int64 ) word->Lo_Name ;
        }
        word->Lo_CfrTilWord = word ;
...
_LO_New ( uint64 ltype, uint64 ctype, byte * value, Word * word, uint64 allocType )
{
    ...
    ListObject * l0 = _DObject_New ( word ? word->Name : ( byte* ) "", ( uint64 ) value, ctype, ltype,
...
_LO_Apply_Arg ( ListObject ** pl1, int64 applyRtoL, int64 i )
{
    ...
    if ( l1->LAttribute & ( LIST | LIST_NODE ) )
    {
        ...
        {
            _Compile_PushEspImm ( ( int64 ) * l2->Lo_PtrToValue ) ;
        }
    }
...
_LO_CopyOne ( ListObject * l0, uint64 allocType )
{
    ...
    if ( l0 )
    {
        if ( l0->LAttribute & ( LIST | LIST_NODE ) )
        {
            ...
            if ( l0->LAttribute & LIST_NODE ) l1 = _DataObject_New ( T_LC_NEW, 0, 0, LIST_NODE, LIST_NODE, 0, ( int64 ) l1, 0 ) ;
        }
...
_LC_New ( int64 forceInitFlag )
{
    ...
    if ( forceInitFlag || lc ) _LC_Init ( _Q_->OVT_LC, lcNewFlag ) ;
    return _Q_->OVT_LC ;
}
