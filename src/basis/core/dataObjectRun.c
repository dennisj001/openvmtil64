...
Object_Run ( Word * word )
{
    ...
    else if ( ( word->LAttribute & T_LISP_SYMBOL ) || ( word->CAttribute & T_LISP_SYMBOL ) ) //lambda variables are parsed as CAttribute & T_LISP_SYMBOL
    {
        ...
        else CfrTil_Do_Variable ( word ) ;
    }
    else if ( word->CAttribute & DOBJECT ) CfrTil_Do_DynamicObject ( word, ACC ) ;
    else if ( word->CAttribute & ( LITERAL | CONSTANT ) ) CfrTil_Do_LiteralWord ( word ) ;
    else if ( word->CAttribute & OBJECT_FIELD ) CfrTil_Do_ClassField ( word ) ;
    else if ( word->CAttribute & ( NAMESPACE_VARIABLE | THIS | OBJECT | LOCAL_VARIABLE | PARAMETER_VARIABLE ) ) CfrTil_Do_Variable ( word ) ;
...
_Compile_C_TypeDeclaration ( )
{
        ...
        ( byte* ) ",", ( byte* ) ";", ( byte* ) "{", ( GetState ( cntx, C_SYNTAX ) ? ( byte* ) "}" : ( byte* ) "=" ), 0 ) )
    {
        if ( String_Equal ( token, ";" ) )
        {
            ...
            Interpreter_InterpretAToken ( cntx->Interpreter0, token, - 1, - 1 ) ; //_Lexer_->TokenStart_ReadLineIndex, lexer->SC_Index ) ;
            break ;
        }
...
void
CfrTil_Do_ClassField ( Word * word )
{
    Context * cntx = _Context_ ;
    Compiler * compiler = cntx->Compiler0 ;
    ...
    else if ( ! GetState ( cntx->Compiler0, LC_ARG_PARSING ) ) cntx->Interpreter0->BaseObject = 0 ;
...
_Do_Compile_Variable ( Word * word, Boolean rvalueFlag )
{
    ...
    if ( GetState ( cntx, C_SYNTAX | INFIX_MODE ) || GetState ( compiler, LC_ARG_PARSING ) )
    {
        if ( rvalueFlag )
        {
            ...
            // a REGISTER_VARIABLE is already its own rvalue, their lvalue is the reg number
        }
        else
        {
    ...
    {
        //if ( word->CAttribute & REGISTER_VARIABLE ) SetPreHere_ForDebug ( Here ) ;
        //else 
...
Do_Variable ( Word * word )
{
    Context * cntx = _Context_ ;
    Compiler * compiler = cntx->Compiler0 ;
    Boolean rvalueFlag = ! Is_LValue ( cntx, word ) ; 
    if ( ( ! rvalueFlag ) && ( ! compiler->LHS_Word ) ) compiler->LHS_Word = word ;
    if ( CompileMode ) _Do_Compile_Variable ( word, rvalueFlag ) ;
    else
    {
        if ( word->CAttribute & ( OBJECT | THIS ) )
        {
            if ( cntx->Compiler0->AccumulatedOffsetPointer )
            {
                ...
                {
                    if ( rvalueFlag ) DataStack_Push ( * ( int64* ) word->W_PtrToValue + word->AccumulatedOffset ) ;
        ...
        else //if ( word->CAttribute & NAMESPACE_VARIABLE )
        {
            ...
            if ( GetState ( cntx, C_SYNTAX ) ) 
            {
                ...
                else goto done ; // delayed to _CfrTil_C_Infix_EqualOp
            }
    ...
    if ( ( word->CAttribute2 & STRUCT ) || Lexer_IsTokenForwardDotted ( cntx->Lexer0 ) ) Finder_SetQualifyingNamespace ( cntx->Finder0, word->TypeNamespace ) ;
    CfrTil_TypeStackPush ( word ) ;
}

void
CfrTil_Do_Variable ( Word * word )
{
    Context * cntx = _Context_ ;
    Compiler * compiler = cntx->Compiler0 ;
...
void
Do_AccumulatedAddress ( byte * accumulatedAddress, int64 offset, Boolean rvalueFlag )
{
    accumulatedAddress += offset ;
    if ( rvalueFlag ) TOS = ( * ( int64* ) accumulatedAddress ) ; // C rvalue
