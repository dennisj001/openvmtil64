...
void
_Compile_Set_C_LValue_WithImm_ThruReg ( int64 address, int64 value, int64 reg )
{
...
void
_Compile_Get_C_Value_ToReg ( int64 reg, int64 value )
{
   _Compile_MoveImm_To_Reg ( reg, ( int64 ) value, CELL ) ;
}

// nb. necessary for esp/ebp
void
_Compile_Get_FromCAddress_ToReg_ThruReg ( int64 reg, byte * address, int64 thruReg )
{
    _Compile_MoveMem_To_Reg ( reg, (byte*) address, thruReg, CELL ) ;    
}

void
_Compile_Get_FromCAddress_ToReg ( int64 reg, byte * address )
{
...
void
_Compile_Set_CAddress_WithRegValue_ThruReg ( byte * address, int64 reg, int64 thruReg )
{
    _Compile_MoveReg_To_Mem ( reg, address, thruReg, CELL ) ;
}

void
Compile_Peek ( Compiler * compiler, int64 stackReg ) // @
{
...
void
Compile_Store ( Compiler * compiler, int64 stackReg ) // !
{
...
void
Compile_Poke ( Compiler * compiler, int64 stackReg ) // =
{
...
void
Compile_AtEqual ( int64 stackReg ) // !
{
    _Compile_Move_Rm_To_Reg ( EAX, stackReg, 0 ) ;
    _Compile_Move_Rm_To_Reg ( EAX, EAX, 0 ) ;
    _Compile_Move_Rm_To_Reg ( ECX, stackReg, - CELL_SIZE ) ;
