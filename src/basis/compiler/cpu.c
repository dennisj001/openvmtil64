...
_Compile_CpuState_SaveSelected ( Cpu * cpu )
{
    ...
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rsp, RSP, OREG2 ) ; // Rsp //this won't be accurate for the runtime because it is called from C 
...
{
    _Compile_Get_FromCAddress_ToReg_ThruReg ( RSP, ( byte * ) & cpu->Rsp, OREG2 ) ; // r12, r13, rsp, rbp : same problem
...
_Compile_CpuState_Save ( Cpu * cpu )
{
    ...
    // eax, ecx, edx, ebx, esp, ebp, esi, edi
    _Compile_PushReg ( ACC ) ;
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->CPU_OREG, OREG2, ACC ) ; // save operand reg thru accum
    _Compile_PopToReg ( ACC ) ; // restore rax

    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rax, RAX, OREG2 ) ; // eax
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rcx, RCX, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rdx, RDX, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rbx, RBX, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rsp, RSP, OREG2 ) ; // Rsp //this won't be accurate for the runtime because it is called from C 
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rbp, RBP, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rsi, RSI, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->Rdi, RDI, OREG2 ) ; // edx

    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R8d, R8D, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R9d, R9D, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R10d, R10D, OREG2 ) ; // edx
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R11d, R11D, OREG2 ) ; // ebx

    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R12d, R12D, OREG2 ) ; // esp //this won't be accurate for the runtime because it is called from C 
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R13d, R13D, OREG2 ) ; // ebp
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R14d, R14D, OREG2 ) ; // esi
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->R15d, R15D, OREG2 ) ; // edi
    ...
    _Compile_Set_CAddress_WithRegValue_ThruReg ( ( byte* ) & cpu->RFlags, ACC, OREG2 ) ; //flags
    ...
    _Compile_GetRValue_FromLValue_ToReg ( OREG2, ( byte* ) & cpu->CPU_OREG ) ; // our scratch reg
}
...
_Compile_CpuState_Restore ( Cpu * cpu, int64 cStackRegFlag )
{
    ...
    //_Compile_GetRValue_FromLValue_ToReg ( OREG2, ( byte* ) & cpu->CPU_OREG ) ; // restore thru - scratch reg

    if ( cStackRegFlag )
    {
        _Compile_Get_FromCAddress_ToReg_ThruReg ( RSP, ( byte * ) & cpu->Rsp, RAX ) ; // r12, r13, rsp, rbp : same problem
        _Compile_Get_FromCAddress_ToReg_ThruReg ( RBP, ( byte * ) & cpu->Rbp, RAX ) ; // r12, r13, rsp, rbp : same problem
    }
    ...
    _Compile_Get_FromCAddress_ToReg_ThruReg ( R12D, ( byte* ) & cpu->R12d, RAX ) ; // r12, r13, rsp, rbp : same problem
    _Compile_Get_FromCAddress_ToReg_ThruReg ( R13D, ( byte* ) & cpu->R13d, RAX ) ; // r12, r13, rsp, rbp : same problem
    ...
    //_Compile_GetRValue_FromLValue_ToReg ( RAX, ( byte* ) & cpu->CPU_OREG2 ) ; // restore thru - scratch reg
}
