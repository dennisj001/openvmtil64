...
{
    if ( printValueFlag ) //&& GetState ( _LC_, ( PRINT_VALUE ) ) )
    {
...
_LO_PrintOneToString ( ListObject * l0, byte * buffer, int64 in_a_LambdaFlag, int64 printValueFlag )
{
    ...
    if ( l0 )
    {
        ...
        else if ( ( l0 == nil ) || ( l0->LAttribute == T_NIL ) ) { if ( _AtCommandLine ( ) ) snprintf ( ( char* ) buffer, BUFFER_SIZE, " nil" ) ; }
        else if ( l0->LAttribute == true ) {if ( _AtCommandLine ( ) )  snprintf ( ( char* ) buffer, BUFFER_SIZE, " T" ) ;}
        else if ( l0->LAttribute == T_RAW_STRING ) snprintf ( ( char* ) buffer, BUFFER_SIZE, " %s", ( char* ) l0->Lo_Value ) ;
        ...
        else if ( l0->LAttribute & BLOCK )  snprintf ( ( char* ) buffer, BUFFER_SIZE, " %s:#<BLOCK>:0x%016lx", l0->Lo_Name, ( uint64 ) l0->Lo_UInteger ) ;
        ...
        else if ( l0->LAttribute & ( CPRIMITIVE | CFRTIL_WORD ) ) snprintf ( ( char* ) buffer, BUFFER_SIZE, " %s", l0->Lo_Name ) ;
