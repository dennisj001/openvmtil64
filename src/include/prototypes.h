...
void _Compile_Move_Reg_To_Rm(int8 dstRmReg, int8 srcReg, int64 rmRegDisp);
...
void Compile_Call_ToAddressThruReg_TestAlignRSP(byte *address, int8 reg);
...
int64 _Context_StringEqual_PeekNextToken(Context *cntx, byte *check);
...
void _Compiler_CompileAndRecord_PushAccum(Compiler *compiler, int8 reg);
...
void CfrTil_WordList_PushToken(byte *token);
Word *_CfrTil_WordList_TopWord(void);
void _CfrTil_WordList_PopWords(int64 n, int8 initFlag);
...
void _Compile_RspReg_Restore(void);
void _Compile_RspReg_Save(void);
...
uint64 GetRsp(void);
uint64 _GetTestRsp(void);
void _GetTestRsp_(byte *src);
void GetTestRsp_Word(void);
void GetTestRsp_Block(void);
