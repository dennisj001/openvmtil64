...
int64 _Compiler_CheckOptimize(Compiler *compiler, Word *word, int64 _forcedReturn);
int64 Compiler_CheckOptimize(Compiler *compiler, int64 forcedReturn);
...
void _SetHere_To_Word_StackPushRegisterCode(Word *word);
void Word_Check_SetHere_To_StackPushRegisterCode(Word *word);
...
Word *_DataObject_New(uint64 type, Word *word, byte *name, uint64 ctype, uint64 ctype2, uint64 ltype, int64 index, int64 value, int64 startCharRlIndex, int64 scIndex);
...
Word *_CfrTil_LocalWord(byte *name, int64 index, int64 ctype, int64 ctype2, int64 ltype);
...
byte *OVT_CheckRecyclableAllocate(dllist *list, int64 size, int8 checkInUseFlag);
...
int64 dllist_Length(dllist *list);
...
dlnode *_dllist_AddNamedValue(dllist *list, byte *name, int64 value, uint64 allocType);
dobject *_dllist_PushNode(dllist *list, dlnode *node);
...
int64 _dllist_Depth(dllist *list);
int64 _dllist_GetTopValue(dllist *list);
int64 _dllist_SetTopValue(dllist *list, int64 value);
void dllist_Map_FromFirstFlag_Indexed(dllist *list, int64 flag, int64 fromFirst, MapFunction2 mf);
...
Word *_Interpreter_DoWord_Default(Interpreter *interp, Word *word0, int64 tsrli, int64 scratchPadIndex);
void Interpreter_DoPrefixWord(Context *cntx, Interpreter *interp, Word *word);
void _Interpreter_DoWord(Interpreter *interp, Word *word, int64 tokenStartReadLineIndex, int64 scIndex);
...
int64 _Readline_CheckArrayDimensionForVariables(ReadLiner *rl);
int64 _Readline_Is_AtEndOfBlock(ReadLiner *rl0);
...
void ByteArray_SetHere_AndForDebug(ByteArray *ba, byte *index);
...
Word *_Compiler_WordList(Compiler *compiler, int64 n);
Word *Compiler_WordList(int64 n);
...
void CfrTil_WordList_RecycleInit(CfrTil * cfrtil, Boolean force);
...
void _Compiler_CompileAndRecord_Word0_PushReg(Compiler *compiler, int8 reg);
...
void _Debugger_ShowDbgSourceCodeAtAddress(Debugger *debugger, byte *address);
...
void SC_ListClearAddress(dlnode *node, Word *word, byte *address);
void Word_Set_SCA(Word *word);
...
void DebugWordList_ShowAll(Debugger *debugger);
void _Compiler_Show_WordList(byte *prefix, int8 inUseFlag);
void Compiler_Show_WordList(byte *prefix);
void Debugger_Show_InUse_CompilerWordList(Debugger *debugger);
void _DWL_ShowList(dllist *list, int8 fromFirst);
void DWL_ShowList(Word *scWord, int8 fromFirst);
...
void _CfrTil_Init_Recycling_SourceCodeWordList(CfrTil *cfrtil, Word *word);
...
int64 String_CheckWordSize(byte *str, int64 wl, Boolean punctFlag, Boolean rPunctFlag);
...
void SkipPreprocessorCode(Boolean toEndifFlag);
...
void LinuxInit(void);
/* basis/exception.c */
int64 _OpenVmTil_ShowExceptionInfo(Boolean all);
int64 _OVT_Pause(byte *prompt, int64 signalsHandled);
int64 _OpenVmTil_Pause(byte *msg);
void OpenVmTil_Pause(void);
void _OVT_Throw(int64 restartCondition, int8 pauseFlag);
void OpenVmTil_Throw(byte *excptMessage, int64 restartCondition, int64 infoFlag);
...
void Error3(byte *format, byte *one, byte *two, int64 three);
void Error2(byte *format, byte *one, int64 two);
/* basis/types.c */
...
void CpuState_AdjustRdi(Cpu *cpu, uint64 *dsp, Word *word);
void Debugger_AdjustRdi(Debugger *debugger, uint64 *dsp, Word *word);
...
ListObject *_LO_New(uint64 ltype, uint64 ctype, uint64 ctype2, byte *value, Word *word, uint64 allocType, int64 rl_Index, int64 scIndex);
void LO_Quote(void);
