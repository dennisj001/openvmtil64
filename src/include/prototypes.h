...
void _Compile_Stack_Push(Boolean stackReg, int64 reg, int64 obj);
...
void Compiler_Init(Compiler *compiler, uint64 state, int64 flags);
...
void CfrTil_RecycleInit_Compiler_N_M_Node_WordList(void);
...
/* basis/typeCheck.c */
Boolean TSI_TypeCheck_NonTypeVariableSigCode(TSI *tsi, Word *word, byte sigCode);
Boolean TSI_TypeCheck_TypeVariableSigCode(TSI *tsi, Word *word0, Word *word1);
Boolean TSI_TypeCheckAndInfer(TSI *tsi);
TSI *TSI_Init(TSI *tsi, Word *opWord, int64 depth);
TSI *TSI_New(Word *opWord, int64 depth, uint64 allocType);
void CfrTil_Typecheck(Word *opWord);
void TSI_TypeStatus_Print(TSI *tsi);
void TSI_ShowTypeErrorStatus(TSI *tsi);
void TSI_UpdateActualTypeStackRecordingBuffer(TSI *tsi, Word *word, Boolean separatorFlag);
byte *Tsi_ExpandTypeLetterCode(byte typeCode, byte *buffer);
int64 Word_TypeSignatureLength(Word *word);
byte *Word_TypeSignature(Word *word, byte *buffer);
byte *Word_ExpandTypeLetterSignature(Word *word, Boolean returnValueOffFlag);
uint64 Tsi_ConvertTypeSigCodeToAttribute(byte signatureCode);
byte Tsi_ConvertTypeAttributeToTypeLetterCode(uint64 attribute);
byte Tsi_Convert_Word_TypeAttributeToTypeCode(Word *word);
...
int64 _LO_Apply_NonMorphismArg(ListObject **pl1, int64 i);
